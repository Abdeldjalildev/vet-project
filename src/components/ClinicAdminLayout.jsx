...
export default function ClinicAdminLayout({ section, children }) {
  ...
  if (status !== 'ready') return <AdminState message={loadError || t('adminAccessError')} retry={load} />
...
function AdminState({ message, retry }) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-100 px-6 dark:bg-gray-950">
      <section className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm dark:border-gray-800 dark:bg-gray-900">
        <p role="alert" className="text-sm font-semibold text-slate-600 dark:text-gray-300">{message}</p>
        {retry && <button type="button" onClick={retry} className="mt-5 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-bold text-white">{t('retry')}</button>}
      </section>
    </main>
  )
}

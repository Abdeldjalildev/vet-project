        ...
        <div className="flex h-16 items-center justify-between">
          <a href="#hero" className="flex items-center gap-2">
            {clinic.logoUrl ? (
              ...
              <span aria-hidden="true" className="font-bold tracking-tight text-emerald-500 dark:text-emerald-400">🐾</span>
            )}
            <span className="font-bold tracking-tight">{localized(clinic.name, i18n.language)}</span>
          </a>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="rounded-lg border border-slate-200 bg-slate-50 p-2 md:hidden dark:border-gray-700 dark:bg-gray-800"
            >
...
function PublicLinks({ t, mobile = false, onNavigate }) {
  const links = [['#hero', 'navHome'], ['#about', 'navAbout'], ['#services', 'navServices'], ['#faq', 'navFaq'], ['#booking', 'heroBtnBook']]
  return links.map(([href, key]) => (
    <a key={href} href={href} onClick={onNavigate} className={mobile ? 'rounded-xl px-3 py-3 text-sm font-bold hover:bg-slate-100 dark:hover:bg-gray-800' : 'transition-colors hover:text-emerald-500'}>
      {t(key)}
    </a>
  ))
}
